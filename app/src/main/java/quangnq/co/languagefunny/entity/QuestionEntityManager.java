...
public class QuestionEntityManager extends ArrayList<QuestionEntity> implements JICommon, Serializable {
        ...
        for (LessonEntity lessonEntity : lessonEntities) {
            ...
            for (String s : list) {
                ...
                if (arrs.length != 7) {
                    ...
                    questionEntity = new QuestionEntity(arrs[0].trim(), arrs[1].trim(), arrs[2].trim(),
                            arrs[3].trim(), Integer.parseInt(arrs[4].trim()), Integer.parseInt(arrs[5].trim()),
                            Integer.parseInt(arrs[6].trim()), lessonEntity.getPath() + "/" + arrs[0].trim(), lessonEntity);
    ...
    public String createEntityListAll(LessonEntityManager lessonEntities) {
        ArrayList<String> list;
        String[] arrs;
        QuestionEntity questionEntity;
        for (LessonEntity lessonEntity : lessonEntities) {
    ...
    public void createAllQuestion(LearningTypeEntity learningTypeEntity) {
        this.createEntityListAll(LessonEntityManager.createListEntity(learningTypeEntity));
    }
}
